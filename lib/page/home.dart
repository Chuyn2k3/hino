...
import 'package:hino/page/home_dashboard.dart';
import 'package:hino/page/home_driver.dart';
import 'package:hino/page/home_realtime.dart';
import 'package:hino/page/home_settings.dart';
...
class _PageState extends State<HomePage> {
  ...
  checkNotiSetting() async {
    ...
    if (Platform.isAndroid) {
      DeviceInfoPlugin().androidInfo.then((androidInfo) => {
            os += androidInfo.version.release.toString() + " ",
            os += androidInfo.version.sdkInt.toString() + " ",
            os += androidInfo.manufacturer.toString() + " ",
            os += androidInfo.model.toString() + " ",
      ...
      DeviceInfoPlugin().iosInfo.then((iosInfo) => {
            os += iosInfo.systemName.toString() + " ",
            os += iosInfo.systemVersion.toString() + " ",
            os += iosInfo.name.toString() + " ",
            os += iosInfo.model.toString() + " ",
  ...
  Future<void> checkForceUpdate() async {
    try {
      final response = await Api.get(context, Api.version); // Gọi API version
      print("version $response");
      if (response != null &&
          response['result'] != null &&
          response['result']['version'] != null) {
  ...
  int _compareVersion(String v1, String v2) {
    ...
    for (int i = 0; i < maxLength; i++) {
      int ai = i < a.length ? int.tryParse(a[i]) ?? 0 : 0;
      int bi = i < b.length ? int.tryParse(b[i]) ?? 0 : 0;
      if (ai > bi) return 1;
      if (ai < bi) return -1;
    }
    return 0;
  ...
  void _showForceUpdateDialog() {
    showDialog(
      context: context,
      barrierDismissible: false, // Không được phép chạm ngoài để đóng
      builder: (context) => WillPopScope(
        onWillPop: () async => false, // Không cho back
        child: AlertDialog(
          title: Text("Cập nhật ứng dụng"),
          content:
              Text("Vui lòng cập nhật phiên bản mới nhất để tiếp tục sử dụng."),
          actions: [
            TextButton(
              onPressed: () async {
                          ...
                          items: [
                            BottomNavigationBarItem(
                              activeIcon: SvgPicture.asset(
                                "assets/images/Fix Icon Hino25.svg",
                                color: ColorCustom.blue,
                              ),
                              icon: Container(
                            ...
                            BottomNavigationBarItem(
                              activeIcon: SvgPicture.asset(
                                "assets/images/Fix Icon Hino33.svg",
                                color: ColorCustom.blue,
                              ),
                              icon: Container(
                                child: SvgPicture.asset(
                            ...
                            BottomNavigationBarItem(
                              activeIcon: SvgPicture.asset(
                                "assets/images/Fix Icon Hino15.svg",
                                width: 30,
                                height: 30,
                                color: ColorCustom.blue,
                              ),
