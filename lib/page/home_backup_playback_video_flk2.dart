...
import 'package:hino/page/home_backup_playback_video_flk_maps.dart';
import 'package:hino/utils/color_custom.dart';
import 'package:hino/widget/actionbar_back.dart';
import 'package:hino/widget/map_playback.dart';
...
import '../widget/video_player_controller.dart';
...
class _PageState extends State<HomeBackupPlaybackVideoPageFlk2> {
  ...
  void initState() {
    ...
    getData2(context);
    timerVideo =
        Timer.periodic(Duration(seconds: 1), (Timer t) => listenerTimer());
    super.initState();
  }
  ...
  void listenerTimer() async {
    ...
    if (_controller != null && _controller.isPlayable()) {
      ...
      setState(() {
        sliderValue = _controller.currentPos.inSeconds;
        position = _controller.currentPos.inSeconds.toString();
        _currentPos = _controller.currentPos;
      });
  ...
  snapshot() async {
    ...
    if (channel?.player != null) {
      String name = DateFormat('ddMMyyyyHHmm').format(DateTime.now()) +
          "Cam" +
