...
class NfcHelper {
  ...
  /// Phân tích dữ liệu từ thẻ — không throw lỗi nếu checksum sai
  static DriverCardData parseCardData(BuildContext context, Uint8List rawData) {
    if (rawData.length < 60) {
      DebugHelper.show(
          context, '❌ Dữ liệu không đủ 60 byte (${rawData.length})');
      return DriverCardData(isValid: false);
    }
    ...
    if (licenseChecksum != rawData[15]) {
      valid = false;
      print(
          "⚠️ GPLX checksum mismatch: calc=$licenseChecksum, tag=${rawData[15]}");
    }
    ...
    if (nameChecksum != rawData[59]) {
      valid = false;
      print(
          "⚠️ Name checksum mismatch: calc=$nameChecksum, tag=${rawData[59]}");
    }
    ...
    if (!valid) {
      DebugHelper.show(context, "⚠️ Dữ liệu có thể chưa ghi đủ (checksum sai)");
    }
  ...
  static Uint8List encodeCardData(
    ...
    try {
      ...
      print('❌ Lỗi mã hóa dữ liệu thẻ: $e');
    }
  ...
  static Future<int> getCardCapacity(dynamic tag) async {
    try {
      ...
      print('⚠️ Error getting card capacity: $e');
      return 64;
    }
  }
  ...
  static Future<void> writeCard({
    ...
    try {
              ...
              try {
                if (Platform.isAndroid) {
                  final cmd = Uint8List.fromList(
                      [0x22, 0x21, ...identifier, i, ...blockData]);
                  await nfcTag.transceive(data: cmd);
                } else {
                  await nfcTag.writeSingleBlock(
                    requestFlags: {Iso15693RequestFlag.highDataRate},
                    blockNumber: i,
                ...
                print('⚠️ Lỗi ghi block $i: $e');
              }
            }

            // --- Ghi lại 2 block chứa checksum ---
            int block15 = 15 ~/ 4; // chứa byte 15
            int block59 = 59 ~/ 4; // chứa byte 59
            for (var blockIndex in [block15, block59]) {
              try {
                Uint8List blockData =
                    dataToWrite.sublist(blockIndex * 4, blockIndex * 4 + 4);
                if (Platform.isAndroid) {
                  final cmd = Uint8List.fromList(
                      [0x22, 0x21, ...identifier, blockIndex, ...blockData]);
                  await nfcTag.transceive(data: cmd);
                } else {
                  await nfcTag.writeSingleBlock(
                    requestFlags: {Iso15693RequestFlag.highDataRate},
                ...
                print('✅ Rewrote checksum block $blockIndex');
              } catch (e) {
                print('⚠️ Lỗi ghi lại checksum block $blockIndex: $e');
              }
            }

            onStatus?.call("✅ Ghi dữ liệu hoàn tất (đã cập nhật checksum)");
            await NfcManager.instance.stopSession();
          } catch (e) {
            print('❌ Lỗi ghi thẻ: $e');
            onError?.call('Chưa ghi được thẻ, vui lòng đặt lại thẻ');
      ...
      print('⚠️ Lỗi khởi động NFC: $e');
      onError?.call('Không thể ghi thẻ');
    }
  }
}
