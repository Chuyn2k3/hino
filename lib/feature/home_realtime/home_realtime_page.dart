...
void _animateToVehicle(Vehicle v) {
  ...
  Widget _buildDetailView() {
    if (!isShowDetail || vehicleClick == null) return Container();
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Image.asset("assets/images/line.png", width: 40, height: 5),
            Row(
              children: [
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Row(
                    children: [
                      const Spacer(flex: 1),
                      Expanded(flex: 8, child: _buildSearchBar()),
                      const Spacer(flex: 2),
                    ],
                  ),
                ),
  ...
  Widget _buildSpeedDial() {
    return Container(
      ...
      child: SpeedDial(
        activeChild: SvgPicture.asset("assets/images/Fix Icon Hino19.svg"),
        renderOverlay: false,
        backgroundColor: Colors.white,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          FloatingActionButton(
            foregroundColor:
                listVehicle.isNotEmpty ? ColorCustom.blue : Colors.transparent,
            backgroundColor:
                listVehicle.isNotEmpty ? ColorCustom.blue : Colors.transparent,
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            const SizedBox(width: 10),
            Image.asset("assets/images/icon_factory.png",
                height: 60, width: 60),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setModalState) {
            List<Vehicle> listSearch = List.from(listCar);
                      ...
                      if (value.isEmpty) {
                        ...
                        for (Vehicle v in listCar) {
                          if (v.info!.vin_no!.contains(value) ||
                              v.info!.licenseplate!.contains(value) ||
                              v.info!.licenseprov!.contains(value) ||
                              v.info!.vehicle_name!.contains(value)) {
                            listSearch.add(v);
                          }
                        }
                        listSearch =
...
// import 'dart:io';
// import 'dart:typed_data';
// import 'dart:ui' as ui; // Alias dart:ui to ui
// import 'package:flutter/material.dart';
// import 'package:flutter/rendering.dart';
// import 'package:flutter/services.dart';
// import 'package:flutter_cache_manager/flutter_cache_manager.dart';
...
// import 'package:hino/feature/home_car/home_car.dart';
// import 'package:hino/feature/home_noti/home_noti.dart';
// // import 'package:hino/feature/home_realtime/widget/vehicle_marker_widget.dart'; // This widget is no longer directly used for marker generation
// import 'package:hino/localization/language/languages.dart';
// import 'package:hino/model/factory.dart';
// import 'package:hino/model/marker_icon.dart';
// import 'package:hino/model/place.dart';
...
//   Timer? _apiTimer;
//   Timer?
//       _debounce; // This timer is not currently used, but kept for consistency.
//   List<Vehicle> _lastVehicleList = [];
//   bool _isApiLoading = false;
...
//   int _polylineIdCounter = 1;
//   Timer? _zoomDebounce;
//   // List<Vehicle> cachedRealtimeVehicles = []; // This variable is not used.
//   // DateTime? lastRealtimeFetch; // This variable is not used.
//   final Map<String, BitmapDescriptor> _markerIconCache = {};
//   final Map<String, BitmapDescriptor> _factoryIconCache =
//       {}; // Cache for factory icons
//   Timer? _searchDebounce;
//   int _maxCacheSize = 200; // Max size for vehicle marker cache
...
//   void _initializeApp() {
//     // Listen to notification stream to trigger UI updates
//     notiController.stream.listen((event) {
//       _safeSetState();
//     });
...
//     // Load initial data or refresh pins if data already exists
//     if (listVehicle.isEmpty) {
//       _loadInitialData();
//     } else {
//       _updatePinRefresh();
...
//   void _startApiTimer() {
//     // Periodically fetch and update data every 30 seconds
//     _apiTimer = Timer.periodic(const Duration(seconds: 30), (timer) {
//       _fetchAndUpdateData();
//     });
//   }
...
//   Future<void> _loadInitialData() async {
//     if (_isApiLoading) return; // Prevent multiple simultaneous API calls
...
//             .map((a) => Vehicle.fromJson(a))
//             .toList();
//         _lastVehicleList = List.from(listVehicle); // Store for comparison
//         _updatePinRefresh(); // Update markers on map
//       }
//     } finally {
//       if (mounted) {
//         setState(() {
...
//         // Only update if the list of vehicles or their positions have changed
//         if (!_isVehicleListEqual(newList, _lastVehicleList)) {
//           listVehicle = newList;
//           _lastVehicleList = List.from(newList);
//           _updatePinRefresh();
//           _safeSetState(); // Trigger rebuild for other UI elements if needed
//         }
//       }
//     } finally {
//       _isApiLoading = false;
...
//   // Helper to compare two lists of vehicles for changes
//   bool _isVehicleListEqual(List<Vehicle> a, List<Vehicle> b) {
//     if (a.length != b.length) return false;
//     for (int i = 0; i < a.length; i++) {
//       // Compare by VID, latitude, and longitude for quick check
//       if (a[i].info?.vid != b[i].info?.vid ||
//           a[i].gps?.lat != b[i].gps?.lat ||
//           a[i].gps?.lng != b[i].gps?.lng) {
//         return false;
...
//   /// Adds a marker to the cache, implementing a basic LRU strategy.
//   void _addToCache(String key, BitmapDescriptor marker) {
//     if (_markerIconCache.length >= _maxCacheSize) {
//       // Remove the oldest entry (first in insertion order for a regular Map)
//       _markerIconCache.remove(_markerIconCache.keys.first);
//     }
//     _markerIconCache[key] = marker;
//   }

//   /// Creates a BitmapDescriptor for a vehicle marker, optionally including the license plate.
//   /// This method directly draws on a canvas for better performance.
//   Future<BitmapDescriptor> _createMarkerWithLicense(
//     Vehicle v, {
//     required bool showLicense,
//   }) async {
...
//     if (!showLicense) {
//       final marker = BitmapDescriptor.fromBytes(iconBytes);
//       _addToCache(cacheKey, marker);
//       return marker;
//     }

//     // --- Direct Canvas Drawing for combined icon and license plate ---
//     final ui.PictureRecorder pictureRecorder = ui.PictureRecorder();
//     final Canvas canvas = Canvas(pictureRecorder);

//     // Decode the base icon image
//     final ui.Codec codec = await ui.instantiateImageCodec(iconBytes);
//     final ui.FrameInfo frameInfo = await codec.getNextFrame();
//     final ui.Image baseIconImage = frameInfo.image;

//     // Define dimensions for the combined marker
//     const double iconWidth = 150;
//     const double iconHeight = 150;
//     const double licensePlateHeight = 40;
//     const double spacing = 5;
//     const double totalHeight = iconHeight + spacing + licensePlateHeight;
//     const double totalWidth = 180; // Slightly wider than icon for license plate

//     // Draw the base icon, scaled to fit
//     paintImage(
//       canvas: canvas,
//       rect: const Rect.fromLTWH(
//           (totalWidth - iconWidth) / 2, 0, iconWidth, iconHeight),
//       image: baseIconImage,
//       fit: BoxFit.contain,
//     );

//     // Draw the license plate background
//     final Paint backgroundPaint = Paint()
//       ..color = const Color.fromARGB(
//           255, 0, 51, 153); // Dark blue for license plate background
//     final RRect licensePlateRRect = RRect.fromRectAndRadius(
//       const Rect.fromLTWH(
//         -150,
//         // (totalWidth - 110) / 2, // Center horizontally, adjust width as needed
//         iconHeight + spacing,
//         480, // Fixed width for license plate background
//         licensePlateHeight,
//       ),
//       const Radius.circular(4),
//     );
//     canvas.drawRRect(licensePlateRRect, backgroundPaint);

//     // Draw the license plate text
//     TextPainter painter = TextPainter(textDirection: TextDirection.ltr);
//     painter.text = TextSpan(
//       text: v.info!.licenseplate!,
//       style: const TextStyle(
//         color: Colors.white, // White text for license plate
...
//       canvas,
//       Offset(
//         licensePlateRRect.left +
//             (licensePlateRRect.width - painter.width) /
//                 2, // Center text horizontally
//         licensePlateRRect.top +
//             (licensePlateRRect.height - painter.height) /
...
//     final marker = BitmapDescriptor.fromBytes(data.buffer.asUint8List());
//     _addToCache(cacheKey, marker);
//     return marker;
//   }

//   /// Marker builder function for ClusterManager
//   Future<Marker> Function(Cluster<Place>) get _markerBuilder =>
//       (cluster) async {
//         if (cluster.items.length == 1) {
//           final v = cluster.items.first.vehicle!;
...
//     for (Vehicle v in listVehicle) {
//       // If a vehicle was previously clicked and is still in the list, update its details
//       if (isShowDetail &&
//           vehicleClick != null &&
//           vehicleClick!.info!.vid == v.info!.vid!) {
//         vehicleClick = v; // Update the clicked vehicle's data
//         _setRadius(
//             LatLng(v.gps!.lat!, v.gps!.lng!), v.info!.vid!.toString(), 80);
//         mapController?.animateCamera(
//             CameraUpdate.newLatLngZoom(LatLng(v.gps!.lat!, v.gps!.lng!), 16));
...
//     _manager.setItems(
//         listVehicleMarker); // Update cluster manager with new vehicle positions
//   }
...
//       return BitmapDescriptor.fromBytes(data!.buffer.asUint8List());
//     } else {
//       // This part is for factory clusters, but factories are usually not clustered.
//       // If they were, this logic would need to be adapted.
//       // For now, it seems to be a fallback for non-text clusters.
//       var a;
//       for (var p in cluster.items) {
//         if (p.factory != null) {
//           a = await _getPinFactory(p.factory!); // Await here
//         }
//       }
//       return a ??
//           BitmapDescriptor.defaultMarker; // Return default if no factory found
//     }
//   }

//   Future<BitmapDescriptor> _getPinFactory(Factory f) async {
//     // Check cache first
//     if (_factoryIconCache.containsKey(f.url!)) {
//       return _factoryIconCache[f.url!]!;
//     }
...
//     final BitmapDescriptor resizedMarker =
//         BitmapDescriptor.fromBytes(_resizeImage(markerImageBytes));
//     _factoryIconCache[f.url!] = resizedMarker;
//     return resizedMarker;
//   }

//   Uint8List _resizeImage(Uint8List data) {
//     IMG.Image img = IMG.decodeImage(data)!;
//     // Resize to a fixed, reasonable size for map markers
//     // This can be adjusted based on desired visual size and performance
//     IMG.Image resized =
//         IMG.copyResize(img, width: 100, height: 100); // Example fixed size
//     return IMG.encodePng(resized);
//   }
...
//     });
//     print('Manual toggle license: $isLicense');
//     _updatePinRefresh(); // Re-render markers with new license plate visibility
//   }
...
//     _apiTimer?.cancel();
//     _debounce?.cancel();
//     _zoomDebounce?.cancel(); // Cancel zoom debounce timer
//     _searchDebounce?.cancel(); // Cancel search debounce timer
//     super.dispose();
//   }

//   @override
//   Widget build(BuildContext context) {
//     // Using context.read instead of context.watch if only reading once
//     // and not needing rebuilds based on this specific provider change.
//     // However, for selecting a vehicle and animating to it, watch is appropriate
//     // if the selection can happen from other parts of the app.
//     Vehicle? v = context.watch<PageProvider>().is_select_vehicle;
//     if (v != null) {
//       _controller.future.then((value) {
//         _markerVehicleClick(v);
//         // Clear the selected vehicle after processing to avoid re-triggering
//         context.read<PageProvider>().selectVehicle(null);
//       });
//     }
...
//                     _zoomDebounce =
//                         Timer(const Duration(milliseconds: 300), () {
//                       // Only update if license visibility actually changes
//                       if (shouldShowLicense != isLicense) {
//                         setState(() {
//                           isLicense = shouldShowLicense;
//                         });
//                         _updatePinRefresh(); // Rebuild markers with new visibility
//                       }
//                       _manager.onCameraMove(value);
//                     });
//                   },
